...
public class SlideBottomPanel extends LinearLayout {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int i = 1; i < mChildCount; i++) {
            ...
            } else if (childView.getTag() != null && (Integer) childView.getTag() == TAG_BACKGROUND){
    ...
    private boolean superDispatchTouchEvent(MotionEvent ev) {
        Log.d(TAG, "superDispatchTouchEvent");
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        Log.d("dispatchTouchEvent", "" + ret);
        return ret;
    }

    @Override
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        Log.d(TAG, "onInterceptTouchEvent isDragging: " + isDragging);
        return isDragging;
    }
