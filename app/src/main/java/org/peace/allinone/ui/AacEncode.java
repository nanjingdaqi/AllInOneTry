...
public class AacEncode {
    ...
    public AacEncode(String outputPath) {
    ...
    public void offerEncoder(byte[] input, long tm) {
        Log.e("daqi", input.length + " is coming");
...
//            long pts = computePresentationTime(presentationTimeUs);
            mediaCodec.queueInputBuffer(inputBufferIndex, 0, input.length, tm * 1000, 0);
            presentationTimeUs += 1;
        } else {
            Log.d("daqi", "No avail input buffer");
        }
    }
...
//                mediaCodec.signalEndOfInputStream();
            ...
            Log.d("daqi", "deque buffer index: " + outputBufferIndex);
...
//                int outPacketSize = outBitsSize + 7; // 7 is ADTS size
                ...
                if ((bufferInfo.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0 && bufferInfo.size != 0) {
                    ...
                    mediaMuxer.writeSampleData(audioTrackId, outputBuffers[outputBufferIndex], bufferInfo);
                    Log.w("daqi", "write one unit.");
                    mediaCodec.releaseOutputBuffer(outputBufferIndex, false);
                }
//                //添加ADTS头
...
//                mediaCodec.releaseOutputBuffer(outputBufferIndex, false);
                ...
                Log.w("daqi", "mux start");
                mediaFormat = mediaCodec.getOutputFormat();
                audioTrackId = mediaMuxer.addTrack(mediaFormat);
                mediaMuxer.start();
            }
            if (stop && signaledEOS) break;
//            outputBufferIndex = mediaCodec.dequeueOutputBuffer(bufferInfo, 0);
        }
        Log.w("daqi", "drain over");
        latch.countDown();
    }
