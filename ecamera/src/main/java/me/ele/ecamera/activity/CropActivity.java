...
public class CropActivity extends Activity implements OnMatrixChangedListener {
      ...
      public void run() {
        ...
        runOnUiThread(new Runnable() {
          ...
          public void run() {
            try {
              loadingView.hide();
              photoView.setPadding(0, title.getMeasuredHeight(), 0,
                  operationBar.getMeasuredHeight());
  ...
  public void cropImage() {
    ...
    final float actualCropWidth =
        (int) Math.min(originalWidth, originalWidth / matrixWidth * outputSquareSideLength);
    final float actualCropHeight =
        (int) Math.min(originalHeight, originalHeight / matrixHeight * outputSquareSideLength);
