...
public class ImageDecoder {
  ...
  public synchronized static Bitmap decodeUri(Context context, Uri uri, int reqWidth,
      int reqHeight) {
    try {
      ...
      Bitmap outputBitmap =
          Bitmap.createBitmap(originalBitmap, 0, 0, outputWidth, outoutHeight, matrix, true);
  ...
  public static Bitmap getBestSimpledBitmap(byte[] data, ExifInfo exifInfo, int reqWidth,
      int reqHeight) {
    return BitmapFactory.decodeByteArray(data, 0,
        data.length, getBestBitmapOptions(data, exifInfo, reqWidth, reqHeight));
  }

  public static BitmapFactory.Options getBestBitmapOptions(byte[] data, ExifInfo exifInfo,
