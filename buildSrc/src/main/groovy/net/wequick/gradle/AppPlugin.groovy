...
class AppPlugin extends BundlePlugin {
    ...
    void logDepend(Project project) {
        println project.name + ' -> ' + 'final dependencies'
        project.configurations.getNames().each { config ->
            project.configurations.getByName(config).dependencies.each {
                if (it instanceof DefaultSelfResolvingDependency) {
                    println it.getSource().asPath
                } else {
                    println config + ': ' + it
                }
            }
    ...
    protected void configureReleaseVariant(variant) {
        ...
        if (project.name.equals('app.mine')) {
            project.packageRelease.doFirst {
                println 'pR doFirst ------------------'
                println printDirectoryTree(small.aarDir)
            }

            project.packageRelease.doLast {
                println 'pR doLast -------------------'
                println printDirectoryTree(small.aarDir)
            }


            project.assembleRelease.doFirst {
                println 'aR doFirst ------------------'
                println printDirectoryTree(small.aarDir)
            }
...
//        println project.name + ' is hooking variant --------------'
//        println 'packagePath: ' + small.packagePath
//        println 'classesDir: ' + small.classesDir
//        println 'bkClassesDir: ' + small.bkClassesDir
        ...
        small.javac.doLast {
//            println 'javac doLast before ------------------'
//            println 'org classesDir'
//            println printDirectoryTree(it.destinationDir)
